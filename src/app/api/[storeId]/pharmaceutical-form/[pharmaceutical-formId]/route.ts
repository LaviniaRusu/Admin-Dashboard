...
export async function GET(
  req: Request,
  { params }: { params: Promise<{ pharmaceuticalFormId: string }> }
) {
  try {
...
export async function PATCH(
  ...
  {
    params,
  }: { params: Promise<{ storeId: string; pharmaceuticalFormId: string }> }
) {
  try {
    const { storeId, pharmaceuticalFormId } = await params;
